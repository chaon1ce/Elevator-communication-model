    ...
    {   //步骤3：读取管道内消息
        ...
        //----------------------->挂断状态处理
            ...
            /*添加消息到有名管道*/
             ...
             /////////////////////////////////////////////////////////////////////////
             //------------------------->接听状态处理
                if(get_state()==TASK_PHONE_STATE_TALK)//当状态为接听时-->思路：与梯口机phone建立tcp，互相发送文字信息--->server
                {
                         char ipAddr[INET_ADDRSTRLEN];//保存点分十进制的地址
                    int sockfd;/*定义套接口描述符*/
                    struct sockaddr_in addr; /*定义套接口描述符*/
                    int addr_len=sizeof(struct sockaddr_in);
                    char buf3[256];/*发送和接送缓冲区*/
                    if((sockfd = socket(AF_INET,SOCK_STREAM,0))<0)/*创建一个通信接口，建立一个socket*/
                    {
                ...
                /////////////////////////////////////////////////////////////////
                    if(bind(sockfd, (struct sockaddr*)(&addr),sizeof(struct sockaddr))<0)//绑定一个端口，向操作系统申请一个端口
                    {
                        perror("blind error");
                        exit(1);
                    }
                    ...
                    {
                        printf("blind port successful!\n");
                        printf("local port:%d\n",PORT);

                    }

                    if(listen(sockfd,5)<0)/*调用listen函数监听端口号，能同时处理的最大连接请求数为5，对对应端口开始接收数据*/
                    {
                        perror("listen error!");
                        exit(1);
                    }
                    ...
                    int newsockfd;//连接套接字定义
                
                        if((newsockfd=accept(sockfd,(struct sockaddr *)(&addr),&addr_len))<0)/*调用accept接受一个连接请求*/
                        {
                            perror("accept error!");
                        }
                        else
                        ...
                        while (1)//互相发送文字信息，当文字信息为quit时退出
                        {
                            memset(buf3,0,sizeof(buf3));   //清空化buf
                            read(newsockfd,buf3,sizeof(buf3));//接收消息
                        ...
                        memset(buf3,0,sizeof(buf3));   //初始化buf
                            printf("said something to client:");
                            scanf("%s",buf3);
                            write(newsockfd,buf3,sizeof(buf3));//向服务端发送数据
                            if(strncmp(buf3,"bye",3) == 0)
                                    {
                                    printf("server quit\n");                    
                                    break;
